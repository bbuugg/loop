...
Loop 是一个 Chrome 扩展，用于在任意网页上构建、录制和回放自动化操作序列。支持**侧边栏**（无需打开 DevTools）和 **DevTools 面板**两种使用方式。
...
4. 选择 `plugin/` 文件夹
5. 打开任意网页
6. 点击 Chrome 工具栏中的 **Loop** 图标，打开侧边栏

> 也可以按 **F12** 打开 DevTools，点击 **Loop** 标签页使用面板模式。
...
2. 弹出配置弹窗，填写所需字段：
   ...
   - XPath 字段旁有 **🎯** 按钮，可直接在页面上点选元素（见下方 [XPath 拾取器](#xpath-拾取器)）
3. 点击 **OK** 添加步骤
...
对于需要 XPath 选择器的步骤，可以通过可视化点选元素来代替手动输入：

1. 打开步骤弹窗（新增或编辑含 XPath 字段的步骤）
2. 点击 XPath 输入框旁的 **🎯** 按钮
3. 面板最小化，页面进入拾取模式 — 鼠标悬停时元素高亮显示
4. 点击目标元素 — 自动生成 XPath 并填入字段
5. 按 **Escape** 取消拾取

> 拾取器会根据元素的 id、class 或位置索引生成能唯一定位该元素的 XPath。
