      ...
      try {
        ...
        await chrome.scripting.executeScript({
          ...
          func: function installRecorder() {
            ...
            let __lastHoverXPath = null;
            let __lastClickTime = null;
            document.addEventListener('mouseover', function crawlerHover(e) {
              const xpath = getXPath(e.target);
              if (xpath !== __lastHoverXPath) __lastHoverXPath = xpath;
            }, true);
            document.addEventListener('click', function crawlerClick(e) {
              ...
              __lastClickTime = now;
              // click step only
              steps.push({ type: 'click', selector: getXPath(e.target) });
              steps.forEach(step => chrome.runtime.sendMessage({ type: 'recordedStep', step }));
            }, true);
          }
        });
        sendResponse({ ok: true });
